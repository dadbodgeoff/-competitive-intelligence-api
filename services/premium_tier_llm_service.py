#!/usr/bin/env python3
"""
Premium Tier LLM Analysis Service - Enhanced Strategic Analysis
"""
...
class PremiumTierLLMService:
    """Premium LLM processing with strategic analysis"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.max_reviews_per_competitor = 35  # Selected from 150
        self.max_input_tokens = 3000  # Higher for premium
        self.max_output_tokens = 1500  # Higher for premium
        self.timeout_seconds = 45  # Longer timeout for premium
    
    async def analyze_competitors_premium_tier(self, restaurant_name: str, restaurant_location: str, restaurant_category: str, competitors_data: Dict[str, List[Dict]]) -> Dict:
        """Process premium competitor data into strategic insights"""
        ...
        logger.info(f"premium_tier_analysis_started: restaurant={restaurant_name}, competitor_count={len(competitors_data)}, total_reviews={total_reviews}")
        
        if not self.model:
            return self._get_fallback_analysis(restaurant_name, restaurant_category, competitors_data)
        
        try:
            # Premium strategic selection: 35 best from 150
            optimized_data = self._premium_strategic_selection(competitors_data)
            self._last_optimized_data = optimized_data  # Store for evidence
            
            # Call the LLM with premium prompt
            llm_response = await self._call_llm_with_timeout(restaurant_name, restaurant_location, restaurant_category, optimized_data)
            
            if llm_response:
                # Add evidence data to response (all 35 reviews per competitor)
                llm_response['evidence_reviews'] = self._extract_evidence_from_optimized(optimized_data)
                return llm_response
            else:
                logger.warning(f"Premium LLM returned empty response for {restaurant_name}, using fallback")
                return self._get_fallback_analysis(restaurant_name, restaurant_category, optimized_data)
                
        except Exception as e:
            logger.error(f"Premium tier LLM analysis failed for {restaurant_name}: {str(e)}")
            return self._get_fallback_analysis(restaurant_name, restaurant_category, competitors_data)
    
    def _premium_strategic_selection(self, competitors_data: Dict[str, List[Dict]]) -> Dict[str, List[Dict]]:
        """Premium strategic selection: Select best 35 from 150 reviews per competitor"""
        optimized = {}
        ...
        for competitor_id, reviews in competitors_data.items():
            ...
            # Segment by rating for strategic analysis
            ...
            # Premium strategic selection: 15 negative, 15 positive, 5 neutral (35 total)
            selected = []
            selected_negative = negative_reviews[:15]  # Top 15 negative
            selected_positive = positive_reviews[:15]  # Top 15 positive
            selected_neutral = neutral_reviews[:5]     # Top 5 neutral
            ...
            if len(selected) < 35:
                ...
                selected.extend(remaining[:35 - len(selected)])
            ...
            for review in selected:
                ...
                review_data = {
                    ...
                    'rating': rating,
                    'text': review.get('text', '')[:200],  # Longer text for premium
    ...
    def _apply_recency_boost(self, reviews: List[Dict]) -> List[Dict]:
        ...
        for review in reviews:
            ...
            # Apply recency boost (higher for premium)
            if days_old <= 30:
                recency_boost = 0.4
            elif days_old <= 90:
                recency_boost = 0.3
    ...
    def get_cost_estimate(self, competitor_count: int, avg_reviews_per_competitor: int) -> Dict[str, float]:
        """Estimate cost for premium tier analysis"""
        estimated_input_tokens = min((competitor_count * avg_reviews_per_competitor * 50), self.max_input_tokens)
    ...
    def _get_fallback_analysis(self, restaurant_name: str, restaurant_category: str, competitors_data: Optional[Dict] = None) -> Dict:
        ...
        # Premium fallback: 5 insights per competitor
        fallback_insights = []
        
        if competitor_names:
            for i, comp_name in enumerate(competitor_names[:5]):  # Max 5 competitors
                # Generate 5 insights per competitor
                insights_for_competitor = [
                    {
                        "category": "opportunity",
                        "title": f"Service Excellence Opportunity vs {comp_name}",
                        "description": f"{restaurant_name} can differentiate by focusing on faster service and better customer experience than {comp_name}.",
                        "confidence": "medium",
                        "significance": 70.0 + (i * 5),
                        "proof_quote": f"Based on competitive analysis patterns for {comp_name}",
                        "mention_count": max(2, total_reviews // 20),
                        "competitor_source": comp_name
                    },
                    {
                        "category": "watch",
                        "title": f"Monitor {comp_name}'s Strengths",
                        "description": f"Keep track of what customers appreciate about {comp_name} to maintain competitive positioning.",
                        "confidence": "high",
                        "significance": 75.0 + (i * 5),
                        "proof_quote": f"Competitive monitoring for {comp_name}",
                        "mention_count": max(3, total_reviews // 15),
                        "competitor_source": comp_name
                    },
                    {
                        "category": "threat",
                        "title": f"Price Competition from {comp_name}",
                        "description": f"Monitor {comp_name}'s pricing strategy to ensure {restaurant_name} remains competitive.",
                        "confidence": "medium",
                        "significance": 65.0 + (i * 5),
                        "proof_quote": f"Price monitoring for {comp_name}",
                        "mention_count": max(1, total_reviews // 25),
                        "competitor_source": comp_name
                    },
                    {
                        "category": "opportunity",
                        "title": f"Menu Innovation vs {comp_name}",
                        "description": f"Identify menu gaps where {restaurant_name} can offer items that {comp_name} doesn't execute well.",
                        "confidence": "medium",
                        "significance": 68.0 + (i * 5),
                        "proof_quote": f"Menu analysis for {comp_name}",
                        "mention_count": max(2, total_reviews // 18),
                        "competitor_source": comp_name
                    },
                    {
                        "category": "watch",
                        "title": f"Customer Loyalty at {comp_name}",
                        "description": f"Understand what drives repeat customers to {comp_name} to improve {restaurant_name}'s retention.",
                        "confidence": "high",
                        "significance": 72.0 + (i * 5),
                        "proof_quote": f"Loyalty analysis for {comp_name}",
                        "mention_count": max(2, total_reviews // 20),
                        "competitor_source": comp_name
                    }
                ]
                fallback_insights.extend(insights_for_competitor)
        else:
            # Generic premium insights
            fallback_insights = [
                {
                    "category": "opportunity",
                    "title": "Premium Service Excellence",
                    "description": f"{restaurant_name} should focus on exceptional customer service to build competitive advantage in the premium market.",
                    "confidence": "high",
                    "significance": 80.0,
                    "proof_quote": "Premium market analysis principles",
                    "mention_count": 5,
    ...
    async def _call_llm_with_timeout(self, restaurant_name: str, restaurant_location: str, restaurant_category: str, optimized_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            prompt = self._build_premium_prompt(restaurant_name, restaurant_location, restaurant_category, optimized_data)
            
            logger.info(f"Calling Gemini API for premium analysis: {restaurant_name} with {len(optimized_data)} competitors")
            logger.info(f"Premium prompt length: {len(prompt)} characters")
            ...
            try:
                ...
                logger.info(f"Premium LLM response received in {processing_time:.2f}s for {restaurant_name}")
                logger.info(f"Response length: {len(response.text)} characters")
            except Exception as e:
                logger.error(f"Error during premium LLM API call: {str(e)}")
                raise
            ...
            if parsed_response:
                ...
                logger.warning(f"Failed to parse premium LLM response for {restaurant_name}")
                return None
                
        except asyncio.TimeoutError:
            logger.warning(f"Premium LLM call timed out after {self.timeout_seconds}s for {restaurant_name}")
            return None
        except Exception as e:
            logger.error(f"Premium LLM call failed for {restaurant_name}: {str(e)}")
            return None
    
    def _build_premium_prompt(self, restaurant_name: str, restaurant_location: str, restaurant_category: str, optimized_data: Dict) -> str:
        """Build the premium prompt for the LLM"""
        
        # Load the premium prompt template
        prompt_template_path = "prompts/premium_tier_llm_prompt.txt"
        try:
            with open(prompt_template_path, 'r', encoding='utf-8') as f:
                prompt_template = f.read()
        except FileNotFoundError:
            logger.error(f"Premium prompt template not found at {prompt_template_path}")
            prompt_template = self._get_default_premium_prompt_template()
    ...
    def _get_default_premium_prompt_template(self) -> str:
        """Default premium prompt template if file not found"""
        return """# COMPETITIVE ANALYSIS - PREMIUM TIER

Analyze {{COMPETITOR_COUNT}} competitors for {{RESTAURANT_NAME}} ({{RESTAURANT_CATEGORY}} in {{RESTAURANT_LOCATION}}).

For each competitor, generate EXACTLY 5 insights:
1. ONE weakness from negative reviews (1-2⭐)
2. ONE strength from positive reviews (5⭐)
3. ONE opportunity based on gaps
4. ONE threat to monitor
...
Generate EXACTLY {{COMPETITOR_COUNT}} × 5 insights. Focus on strategic business intelligence."""
    
    def _parse_llm_response(self, response_text: str) -> Optional[Dict]:
        """Parse and validate LLM response"""
        try:
            # Save full response for debugging
            with open('premium_llm_response_debug.txt', 'w', encoding='utf-8') as f:
                f.write(response_text)
            logger.info(f"Full premium LLM response saved to premium_llm_response_debug.txt")
            ...
            if '```json' in response_text:
                ...
                if json_start == -1 or json_end == 0:
                    logger.error("No JSON found in premium LLM response")
                    return None
                
                json_text = response_text[json_start:json_end]
            
            logger.info(f"Extracted premium JSON (first 300 chars): {json_text[:300]}")
            parsed = json.loads(json_text)
            
            # Validate structure
            if 'actionable_insights' not in parsed:
                logger.error("Missing 'actionable_insights' in premium LLM response")
                return None
            
            insights = parsed['actionable_insights']
            if not isinstance(insights, list) or len(insights) == 0:
                logger.error("Invalid or empty insights list in premium response")
                return None
            ...
            if not valid_insights:
                logger.error("No valid insights after premium validation")
                return None
            ...
            logger.error(f"Failed to parse JSON from premium LLM response: {str(e)}")
            return None
        except Exception as e:
            logger.error(f"Error parsing premium LLM response: {str(e)}")
            return None
    ...
    def _validate_insight(self, insight: Dict) -> bool:
        ...
        for field in required_fields:
            if field not in insight or not insight[field]:
                logger.warning(f"Premium insight missing required field: {field}")
                return False
        ...
        if 'significance' not in insight:
            insight['significance'] = 70.0  # Higher default for premium
        if 'proof_quote' not in insight:
            insight['proof_quote'] = ''
        if 'mention_count' not in insight:
            insight['mention_count'] = 2  # Higher default for premium
